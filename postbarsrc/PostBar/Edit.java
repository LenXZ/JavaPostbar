...
public class Edit extends JFrame {
	...
	public Edit() { // 构造函数
		...
		textPane.setPreferredSize(new Dimension(158, 200));
		editpanecen = new JPanel();
		editpanecen.setLayout(new BorderLayout());
		editpanecen.add(createJToolBar(actions), BorderLayout.NORTH); // 增加工具栏
		editpanecen.add(new JScrollPane(textPane), BorderLayout.CENTER); // 增加文本窗格
		editpanecen.add(statusBar, BorderLayout.SOUTH); // 增加状态栏
//		init();		
	}
	...
	void init() {
		...
		add(editpanecen);
		setBounds(1000, 200, 800, 600); // 设置窗口尺寸
	...
	class SaveAction extends AbstractAction { // 保存命令
		...
		public void actionPerformed(ActionEvent e) {
			int i = filechooser.showSaveDialog(Edit.this); // 显示保存文件对话框
	...
	class PasteAction1 extends AbstractAction { // 粘贴命令
		public PasteAction1() {
			super("粘贴");
		}
